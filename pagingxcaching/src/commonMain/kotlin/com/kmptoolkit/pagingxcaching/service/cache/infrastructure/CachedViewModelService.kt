...
import com.kmptoolkit.core.util.LifecycleEffect
import com.kmptoolkit.pagingxcaching.service.api.ApiResult
import com.kmptoolkit.pagingxcaching.service.cache.dao.CacheDao
import com.kmptoolkit.pagingxcaching.service.paging.infrastructure.ViewModelService
...
abstract class CachedViewModelService<Key : QueryKey, Network : Any, Local : Any, Output : Any, FlowType>(
    ...
    val refreshAction: CachedRefreshAction = CachedRefreshAction.RefreshAndUpdate,
) : ViewModelService {
    ...
    private fun shouldDelete(): Boolean =
        refreshAction is CachedRefreshAction.RefreshAndDelete || refreshAction is CachedRefreshAction.RefreshAndDeleteOnTime

    private suspend fun shouldRefresh(): Boolean =
        when (refreshAction) {
            CachedRefreshAction.RefreshAndDelete -> true
            is CachedRefreshAction.RefreshAndDeleteOnTime -> shouldRefreshOnTime(refreshAction.cacheTimeoutMillis)
            CachedRefreshAction.RefreshAndUpdate -> true
            is CachedRefreshAction.RefreshAndUpdateOnTime -> shouldRefreshOnTime(refreshAction.cacheTimeoutMillis)
            CachedRefreshAction.RefreshNever -> false
        }
