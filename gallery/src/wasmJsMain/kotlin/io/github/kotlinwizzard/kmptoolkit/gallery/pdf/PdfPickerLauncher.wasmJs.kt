...
internal actual fun LaunchPdfPicker(
    onResult: (List<io.github.kotlinwizzard.kmptoolkit.gallery.pdf.PdfPickerResultData>) -> Unit,
    pdfPickerStatus: io.github.kotlinwizzard.kmptoolkit.gallery.pdf.PdfPickerStatus.LaunchRequested,
...
internal suspend fun List<File>.mapFilesPdf(
    ...
    return mapNotNull { file ->
        runCatching {
            ...
            // 4) pdf.js Document laden (WASM Wrapper statt getDocument(dynamic))
            val loadingTask = getDocument(getDocumentSrcFromBuffer(arrayBuffer))
            val pdfDoc = loadingTask.promise.await<PdfDocument>()

            val totalPages = pdfDoc.numPages

            val page = pdfDoc.getPage(1).await<PdfPage>()

            // 5) Viewport (Wrapper statt page.getViewport(js(...)))
            val viewport = getViewport(page, scale = 1.0)
            ...
            // 6) Render (Wrapper statt page.render(js(...)))
            val renderTask = renderPage(page, ctx, viewport)
            renderTask.promise.await<JsAny>()
        ...
        }.apply { exceptionOrNull()?.let { throw it } }.getOrNull()
    }
}
