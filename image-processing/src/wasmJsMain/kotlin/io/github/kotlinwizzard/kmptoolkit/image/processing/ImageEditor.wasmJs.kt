...
actual object ImageEditor {
    ...
    private suspend fun edit(
        ...
        val imageBitmap = window.createImageBitmap(blob).unsafeCast<Promise<ImageBitmap>>().await<ImageBitmap>()
    ...
    private inline fun applyPerPixelRgba(
        ...
        var i = 0
        while (i + 3 < n) {
            val r = getU8(data, i).toFloat()
            val g = getU8(data, i + 1).toFloat()
            val b = getU8(data, i + 2).toFloat()
            val a = getU8(data, i + 3).toFloat()

            val out = transform(r, g, b, a)

            setU8(data, i, out[0].roundToInt().coerceIn(0, 255))
            setU8(data, i + 1, out[1].roundToInt().coerceIn(0, 255))
            setU8(data, i + 2, out[2].roundToInt().coerceIn(0, 255))
            setU8(data, i + 3, out[3].roundToInt().coerceIn(0, 255))
